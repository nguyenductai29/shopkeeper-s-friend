...
const TableHeader = React.forwardRef<HTMLTableSectionElement, React.HTMLAttributes<HTMLTableSectionElement>>(
  ({ className, ...props }, ref) => <thead ref={ref} className={cn("bg-table-head [&_tr]:border-b", className)} {...props} />,
);
...
const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        "h-10 px-3 text-left align-middle font-mono text-[10px] font-medium uppercase text-muted-foreground [&>button]:uppercase [&:has([role=checkbox])]:pr-0",
        className,
      )}
      {...props}
    />
