...
function Change({ value, inverse = false }: { value?: number | null; inverse?: boolean }) {
  ...
  return <span className={`text-[10px] font-medium ${positive ? "text-success" : "text-destructive"}`}><Icon className="inline h-3 w-3" /> {Math.abs(value).toFixed(1)}%</span>;
}
    ...
    <div className="flex h-full min-h-0 flex-col gap-2 overflow-hidden">
      <header className="flex shrink-0 items-center justify-between gap-3">
        <div className="min-w-0">
          <h1 className="font-display text-[26px] font-extrabold leading-tight">Tổng quan cửa hàng</h1>
      ...
      <section className="grid min-h-0 flex-1 grid-cols-12 gap-2">
        <Card className="col-span-8 flex min-h-0 flex-col p-3">
          ...
          <div className="min-h-0 flex-1"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData} margin={{ top: 5, right: 5, left: -15, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="label" fontSize={9} tickLine={false} /><YAxis fontSize={9} tickFormatter={compactMoney} tickLine={false} /><Tooltip formatter={(v: number) => formatVND(Number(v))} /><Legend wrapperStyle={{ fontSize: 10 }} /><Area type="monotone" dataKey="revenue" name="Doanh thu" stroke="var(--primary)" fill="var(--primary)" fillOpacity={0.12} strokeWidth={2} /><Area type="monotone" dataKey="expenses" name="Chi phí" stroke="var(--accent)" fill="var(--accent)" fillOpacity={0.08} strokeWidth={2} /></AreaChart></ResponsiveContainer></div>
        </Card>

        <div className="col-span-4 grid min-h-0 grid-rows-2 gap-2">
          <RecentTable title="Đơn hàng gần đây" rows={(data?.recentOrders || []).slice(0, 8).map((o) => [o.order_code || "—", o.customer_name || "Khách lẻ", formatVND(Number(o.total)), paymentLabel(o.payment_status)])} />
          <RecentTable title="Hàng nhập gần đây" rows={(data?.recentPurchases || []).slice(0, 8).map((p) => [p.product_code || "—", p.product_name || "Sản phẩm", `${p.quantity} sp`, formatVND(Number(p.total))])} />
        </div>
      </section>
    </div>
  );
...
function RecentTable({ title, rows }: { title: string; rows: string[][] }) {
  return <Card className="flex min-h-0 flex-col overflow-hidden p-3"><h2 className="mb-1 shrink-0 text-sm font-semibold">{title}</h2><div className="grid min-h-0 flex-1 content-start gap-0.5 overflow-auto">{rows.length ? rows.map((r, i) => <div key={i} className="grid grid-cols-[.8fr_1.4fr_1fr_.8fr] items-center gap-2 border-t py-1 text-[10px] first:border-t-0"><span className="truncate font-medium">{r[0]}</span><span className="truncate text-muted-foreground">{r[1]}</span><span className="truncate text-right">{r[2]}</span><span className="truncate text-right text-muted-foreground">{r[3]}</span></div>) : <div className="py-8 text-center text-xs text-muted-foreground">Chưa có dữ liệu</div>}</div></Card>;
}
