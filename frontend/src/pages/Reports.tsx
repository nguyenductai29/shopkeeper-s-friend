...
function ReportsContent() {
  ...
  const currencyOrders = useMemo(() => orders.filter((order) => (order.currency || "VND") === currency), [orders, currency]);
  const currencyItems = useMemo(() => {
    const orderIds = new Set(currencyOrders.map((order) => order.id));
    return items.filter((item) => orderIds.has(item.order_id));
  }, [currencyOrders, items]);
  ...
  const report = useMemo(
    () => buildSalesReport(currencyOrders, currencyItems, period, payment, productCode),
    [currencyOrders, currencyItems, period, payment, productCode],
  );
  const metrics: [string, string, string, LucideIcon][] = [
    ["Doanh thu", formatCurrency(report.total, currency), `${report.count} đơn trong kỳ`, TrendingUp],
    ["Đơn hàng", String(report.count), payment === "all" ? "Đã trả và chưa trả" : payment === "paid" ? "Đã thanh toán" : "Chưa thanh toán", ReceiptText],
    ["Trung bình", formatCurrency(report.average, currency), "Mỗi đơn hàng", WalletCards],
    ["Lợi nhuận gộp", formatCurrency(report.grossProfit, currency), "Doanh thu trừ giá vốn", TrendingUp],
  ];

  const exportCsv = () => {
    const rows = [
      ["Ngày", "Mã đơn", "Khách hàng", "Sản phẩm", "Thanh toán", `Doanh thu (${currency})`, `Giá vốn (${currency})`, `Lợi nhuận gộp (${currency})`],
      ...report.rows.map((row) => [
    ...
    link.href = url;
    link.download = `bao-cao-doanh-thu-${currency.toLowerCase()}.csv`;
    link.click();
    window.setTimeout(() => URL.revokeObjectURL(url), 0);
  };

  const statusMessages = (error || itemsUnavailable) && (
  ...
  return (
    <div className="h-full min-h-0 min-w-0 overflow-hidden">
      <div className="flex h-full min-h-0 min-w-0 flex-col gap-3 p-3 sm:p-4">
        ...
        <div className="flex shrink-0 gap-2 overflow-x-auto pb-1">
          <select value={currency} onChange={(event) => { setCurrency(event.target.value); setProductCode("all"); }} className="h-9 w-24 shrink-0 rounded-md border bg-card px-2 text-sm" aria-label="Đơn vị tiền báo cáo">
            <option value="VND">VND</option><option value="JPY">JPY</option>
          </select>
          ...
          <Select value={payment} onValueChange={(value: PaymentFilter) => setPayment(value)}>
            <SelectTrigger className="w-44 shrink-0 bg-card"><SelectValue /></SelectTrigger>
            <SelectContent>
          ...
          <Select value={productCode} onValueChange={setProductCode} disabled={itemsUnavailable || products.length === 0}>
            <SelectTrigger className="w-48 shrink-0 bg-card"><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <section className="grid shrink-0 grid-cols-[repeat(4,minmax(155px,1fr))] gap-3 overflow-x-auto pb-1" aria-label="Chỉ số doanh thu">
          {metrics.map(([label, value, note, Icon], index) => (
            <div key={label} className="animate-rise min-w-0 rounded-lg border bg-card p-3 backdrop-blur-md" style={{ animationDelay: `${index * 60}ms` }}>
              ...
              <p title={value} className="mt-1 truncate font-display text-xl font-extrabold">{value}</p>
              <p className="mt-1 font-mono text-[10px] text-muted-foreground">{note}</p>
            </div>
          ))}
        </section>

        <Tabs defaultValue="trend" className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
          <TabsList className="shrink-0 self-start">
            <TabsTrigger value="trend">Xu hướng</TabsTrigger>
            <TabsTrigger value="details">Chi tiết doanh thu</TabsTrigger>
          </TabsList>
        <TabsContent value="trend" className="flex min-h-0 min-w-0 flex-1 flex-col overflow-y-auto overscroll-contain rounded-lg border bg-card p-3 backdrop-blur-md data-[state=inactive]:hidden">
          {statusMessages && <div className="mb-3 shrink-0">{statusMessages}</div>}
          <div className="mb-3 shrink-0"><h2 className="font-display text-[15px] font-bold">Xu hướng doanh thu</h2><p className="font-mono text-[10px] text-muted-foreground">Dữ liệu theo {periodLabels[period]} · {currency}</p></div>
          <div className="min-h-[160px] w-full flex-1">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={report.points}>
                ...
                <YAxis axisLine={false} tickLine={false} fontSize={10} tickFormatter={(value: number) => new Intl.NumberFormat("vi-VN", { notation: "compact", maximumFractionDigits: 1 }).format(value)} />
                <Tooltip formatter={(value: number) => formatCurrency(Number(value), currency)} contentStyle={{ background: "oklch(0.985 0.014 54)", border: "1px solid oklch(0.25 0.018 40 / 0.1)", borderRadius: "6px" }} />
        ...
        <TabsContent value="details" className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-lg border bg-card backdrop-blur-md data-[state=inactive]:hidden">
          <div className="shrink-0 border-b p-3"><h2 className="font-display text-[15px] font-bold">Chi tiết doanh thu</h2><p className="font-mono text-[10px] text-muted-foreground">{report.count} đơn trong kỳ</p></div>
          <div className="min-h-0 flex-1 overflow-auto overscroll-contain">
            {statusMessages && <div className="p-3">{statusMessages}</div>}
            <table className="w-full min-w-[650px] text-left text-[12px]">
              <thead className="sticky top-0 z-10 bg-muted font-mono text-[9px] uppercase text-muted-foreground"><tr><th className="p-3">Ngày</th><th>Khách hàng</th><th>Sản phẩm nổi bật</th><th>Mã đơn</th><th className="pr-3 text-right">Doanh thu</th></tr></thead>
              <tbody className="divide-y">
                {report.rows.map((row) => (
                  <tr key={row.order.id} className="hover:bg-muted/40">
                    ...
                    <td className="pr-3 text-right font-mono font-semibold">{formatCurrency(row.revenue, currency)}</td>
                  </tr>
                ))}
